    ...
    def get_zones(self, zone_id=None, **kwargs):
        try:
            ...
            if request.httprequest.method == 'GET':
                if zone_id:
                    zone = request.env['ddn.zone'].sudo().browse(zone_id)
                    if not zone.exists():
                ...
                zones = request.env['ddn.zone'].sudo().search([])
            ...
            if request.httprequest.method == 'POST':
                ...
                zone = request.env['ddn.zone'].sudo().create({'name': name})
        ...
        try:
            ...
            if request.httprequest.method == 'GET':
                if ward_id:
                    ...
                    ward = request.env['ddn.ward'].sudo().browse(ward_id)
                    if not ward.exists():
                ...
                # Fetch all wards
                wards = request.env['ddn.ward'].sudo().search([])
